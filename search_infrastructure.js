  ...
  try {
    ...
    if (data.tool_calls) {
      for (let tc of data.tool_calls) {
        ...
        if (args.includes("INFRASTRUCTURE")) {
          console.log("Found in tool call:", tc.function.name);
          if (tc.function.name === 'default_api:write_to_file') {
             fs.writeFileSync('extracted_page2.tsx', JSON.parse(tc.function.arguments).CodeContent);
             console.log("Extracted to extracted_page2.tsx");
          }
        }
      }
    }
