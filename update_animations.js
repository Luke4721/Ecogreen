...
const oldImpact = /<section className="py-20 bg-slate-800 relative z-20 shadow-lg border-b border-slate-700">.*?Redefining Green Metals: Iron • Copper • Lithium • Cobalt • Aluminum\\s*<\\/div>\\s*<\\/div>\\s*<\\/section>/s;
...
const newImpact = [
  ...
  '                      <CountUp end={impact.val} duration={2.5} separator="," enableScrollSpy={true} scrollSpyOnce={true} scrollSpyDelay={100} />',
  '                    </h3>',
...
code = code.replace(oldImpact, newImpact);

const oldStrip = /\\{\\/\\* CLIENTS STRIP \\*\\/\\}.*?<\\/section>/s;
...
const clientLogosJSX = clientLogos.map(client => 
  ...
  '                  <img src="https://logo.clearbit.com/' + client.domain + '" alt="' + client.name + '" className="h-8 w-8 object-contain" onError={(e) => { e.target.src = \\'https://ui-avatars.com/api/?name=\\' + \\'' + client.name + '\\' + \\'&background=random&color=fff\\' }} />' +
...
console.log('Page updated with CountUp and Infinite Marquee');
