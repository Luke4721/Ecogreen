...
function build() {
  ...
  if (code.includes(oldImageString)) {
    code = code.replace(oldImageString, newHeroBg);
  } else {
    // try removing carriage returns just in case
    const oldImageString2 = `<div \n            className="absolute inset-0 opacity-40 bg-cover bg-center"\n            style={{ backgroundImage: "url('https://goecogreen.in/wp-content/uploads/2026/05/eco_green_home_cover_img.webp')" }}\n          ></div>`;
    code = code.replace(oldImageString2, newHeroBg);
  }

  // Fix Hero overlap by adding pt-32 to the container
  code = code.replace('<div className="container mx-auto px-4 relative z-10 text-center max-w-4xl">', '<div className="container mx-auto px-4 relative z-10 text-center max-w-4xl pt-32">');

  // Fix logo src
  code = code.replace(/<div className="w-10 h-10 bg-green-600 rounded-full flex items-center justify-center">[\s\S]*?<span className="text-white font-bold text-xl">EG<\/span>[\s\S]*?<\/div>[\s\S]*?<span className="font-bold text-2xl text-gray-800 tracking-tight">Eco Green<\/span>/, 
  '<img src="/logo.png" alt="EcoGreen Logo" className="h-10 w-auto object-contain" />');

  // Fix footer logo src
  code = code.replace(/<div className="w-10 h-10 bg-green-500 rounded-full flex items-center justify-center">[\s\S]*?<span className="text-white font-bold text-xl">EG<\/span>[\s\S]*?<\/div>[\s\S]*?<span className="font-bold text-2xl text-white tracking-tight">Eco Green<\/span>/, 
  '<img src="/logo.png" alt="EcoGreen Logo" className="h-10 w-auto object-contain brightness-125" />');
